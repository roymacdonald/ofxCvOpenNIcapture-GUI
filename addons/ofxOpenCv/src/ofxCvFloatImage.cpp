...
//--------------------------------------------------------------------------------
void ofxCvFloatImage::setMinMax(){

	float min = 1.0;
	float max= 0;
	for (int y =0; y<cvImage->height; y++) {
		float * ptr = (float*)(cvImage->imageData + y *cvImage->widthStep);
...
void ofxCvFloatImage::setFromPixels( const unsigned char* _pixels, int w, int h ) {
	...
	cout << "w: "<<ofToString(w)<<"  width: "<<ofToString(this->width)<<"   h: "<<ofToString(h)<<"  height: "<<ofToString(this->height)<<endl;
    if( w == width && h == height ) {
        ...
        ofLog(OF_LOG_ERROR, "in ofxCvFloatImage setFromPixels, size mismatch");
    }
}
